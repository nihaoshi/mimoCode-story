 ...
 * 
 * 用法：
 *   node step-guard.js pre  <dimension> <workflow-dir> <project-dir>
 *   node step-guard.js post <dimension> <workflow-dir>
 * 
 * 维度：role, item, env, timeline, foreshadow, supply, repeat, cross-volume, storyline, setting
 * 退出码：0=通过，1=失败（阻断）
 */
...
const VALID_DIMENSIONS = ['role', 'item', 'env', 'timeline', 'foreshadow', 'supply', 'repeat', 'cross-volume', 'storyline', 'setting'];

if (!action || !dimension) {
  console.error('用法: node step-guard.js <pre|post> <dimension> <workflow-dir> <project-dir>');
...
const OUTPUT_FILES = {
  ...
  supply: 'audit-supply.json',
  repeat: 'audit-repeat.json',
  'cross-volume': 'audit-cross-volume.json',
  storyline: 'audit-storyline.json',
  setting: 'audit-setting.json',
};
