...
- **检查范围**：动态扫描项目目录下所有配置文件（设定/、追踪/、故事线/、跨卷追踪/、大纲/ 下的所有 .md 文件）
- **检查内容**：
  1. 目录是否存在（设定/、追踪/、大纲/、正文/、故事线/、跨卷追踪/）
  2. 文件是否完整（所有必需配置文件是否存在）
  3. 文件内容是否有效（是否有实质内容，非空模板）
  4. 一致性是否通过（设定与追踪文件是否一致）
...
- **执行流程**：
  1. 运行检查 → 获得结果
  2. 有任何 fail/warn/block → 列出所有问题
  3. 逐一修复所有问题
  4. 重新检查 → 直到全部 pass 才能继续

**唯一允许跳过的情况**：用户明确说"跳过健康检查"，且用户已确认项目结构完整。
