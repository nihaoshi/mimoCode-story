...
**整个流程任何环节的检查结果都必须为 `pass` 才能继续。**

- **检查范围**：项目健康检查、质量门禁、一致性检查、设定回写验证等所有检查环节
- **阻断规则**：
  - `fail`（缺失/无效）→ **必须修复，不能跳过**
  - `warn`（警告）→ **必须修复，不能跳过**
  - `block`（阻断）→ **必须修复，不能跳过**
- **执行流程**：检查 → 有任何问题 → 列出所有问题 → 逐一修复 → 重新检查 → 直到全部 pass

**唯一允许跳过的情况**：用户明确说"跳过XX检查"，且用户已确认项目状态完整。
