...
- **output-contract.md** = 输出契约 SSOT：定义 Stage → 文件映射 + `_meta.json` schema（含 `structure_counts`）+ 下游 `story-short-write-mimo` 消费规范 + Phase 7 门控接入点。与 `story-short-write-mimo` 副本字节一致，由 `scripts/check-shared-files.sh` 守护
