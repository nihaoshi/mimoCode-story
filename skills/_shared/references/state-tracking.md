...
- **待回收伏笔**：{与该角色相关的未回收伏笔}
- **性格锚点**（跨章节一致性参考，不轻易变更）：
  - 核心性格：{2-3个关键词，如"冷静/护短/毒舌"}
  - 说话风格：{如"话少但毒、从不废话、爱用反问"}
  - 行为模式：{如"遇事先想后做、对敌人绝不手软、对朋友掏心掏肺"}
  - 禁忌：{如"不会主动求助、不会在人前示弱、不会说对不起"}
- **状态变更记录**：
  - 第{N}章：{变化描述}
```
...
**目的**：防止 AI 在不同章节把同一角色写成不同性格。

1. **写对话前必读**：每章写某个角色的对话前，检查该角色的"性格锚点"——说话风格和禁忌
2. **行为一致性**：角色的"行为模式"是底线，不能因为剧情需要突然违反（除非有充分的剧情铺垫）
3. **性格锚点变更条件**：只有当角色经历了重大事件（生死、背叛、顿悟等）且在正文中充分展示后，才允许修改性格锚点。修改时必须在"状态变更记录"中标注原因
4. **示例**：如果角色锚点是"遇事先想后做"，那么在紧张场景中也不能让他突然冲动行事，除非前面有铺垫他性格转变的过程
