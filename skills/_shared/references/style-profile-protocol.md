...
> **何时加载**：story-long-analyze-mimo Stage 6 执行前。下游写作 skill 直接读 `文风.md`，不加载本协议。
...
- **读**：story-import-mimo 同步到 `{项目}/对标/{书名}/文风.md`；story-long-write-mimo 读项目对标视图（回退拆文库）
...
精确决议表见 `narrative-writer 写作代理` 的“被调用协议”段。
