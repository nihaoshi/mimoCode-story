...
# story-chapter-ultra-write-mimo → story-write-mimo
...
```
/story-chapter-ultra-write-mimo [参数]
↓ 转发为
/story-write-mimo ultra [参数]
```
...
actor({
  operation: {
    ...
    prompt: `
      用户触发了 story-chapter-ultra-write-mimo，需要转发到 story-write-mimo。

      用户输入：{用户原始输入}

      请执行 story-write-mimo skill，使用 ultra 模式。
      如果指定了章节号，写指定章节。
