...
- story-import-mimo 把整个 `拆文库/{书名}/` 同步到项目 `{项目}/对标/{书名}/` 时**自动包含**文风（与拆文报告同等待遇）
- 写作端（story-long-write-mimo）的日更循环读 `{项目}/对标/{书名}/文风.md`（按对标书路径查找规则，回退 `拆文库/{书名}/`）

## 重生策略（旧拆文库无文风）

旧 `拆文库/{书名}/` 没有文风文件时：

- **完整重跑** `/story-long-analyze-mimo`：开销大，会重跑 Stage 0-5（不必要）
