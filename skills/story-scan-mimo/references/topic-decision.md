...
Phase 4 用本文方法产出 `选题决策.md`——市场角度的选题初稿："能爆的原因"先当假设，拆文（story-long-analyze-mimo）后回填验证，写作（story-long-write-mimo）开书时直接用。
...
2. 告知用户路径，并说明下一步：「开书时把 `选题决策.md` 放到小说项目根目录，写作会自动读取；想确认"能爆的原因"就先 `/story-long-analyze-mimo` 拆对标书，拆完会回填。」
