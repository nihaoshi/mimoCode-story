...
version: 3.0.0
description: |
  单章写作（已合并到 story-write-mimo）。
  本skill已重构为转发入口，所有写作功能统一由 story-write-mimo 提供。
  触发方式：/story-chapter-write-mimo、/chapter-write、写第X章、单章写作、续写
category: write
triggers:
  - /chapter-write
  - 写第X章
...
# story-chapter-write-mimo → story-write-mimo
...
actor({
  operation: {
    ...
    prompt: `
      用户触发了 story-chapter-write-mimo，需要转发到 story-write-mimo。

      用户输入：{用户原始输入}

      请执行 story-write-mimo skill，使用 standard 模式。
      如果指定了章节号，写指定章节。
