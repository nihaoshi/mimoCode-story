...
### Step 08: 读取上下文
- ❌ 偷懒：用摘要代替原文、跳过设定文件、跳过跨卷追踪和故事线
- ✅ 正确：上一章结尾必须是最后 500 字原文；设定文件必须全部读取；跨卷追踪（3个）和故事线（3个）必须加载
- 🔍 验证：`step08-context.json` 中 previous_chapter_ending 长度 >= 100，设定文件已加载，跨卷追踪和故事线已加载
