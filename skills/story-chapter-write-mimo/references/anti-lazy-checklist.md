...
### Step 08: 读取上下文
- ❌ 偷懒：用摘要代替原文、跳过设定文件
- ✅ 正确：上一章结尾必须是最后 500 字原文；设定文件（世界观、势力、关系、题材、文风）必须全部读取
- 🔍 验证：`step08-context.json` 中 previous_chapter_ending 长度 >= 100，设定文件已加载
...
### Step 11: 综合质量检测
- ❌ 偷懒：跳过某些检测项、忽略 WARN、跳过设定校验
- ✅ 正确：运行全部 7 项检测（字数+禁用词+AI标点+一致性+设定校验+逻辑+跨章），记录所有问题
...
### Step 14: 追踪+设定更新
- ❌ 偷懒：凭记忆更新、忽略设定变化
- ✅ 正确：从正文实际提取信息，设定有变化必须更新
- 🔍 验证：7 个追踪文件已更新，设定文件有变化时已更新
