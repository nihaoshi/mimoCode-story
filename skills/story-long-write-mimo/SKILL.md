...
1. **检查细纲 + 读取上下文**（⚠️ 不可跳过，不可偷懒，不可省略任何一项）：读取 `大纲/细纲_第{N}章.md`。如果不存在，**必须先补建细纲再写正文**，不允许跳过细纲直接写作。补建时参考卷纲中本章对应的事件规划和上下文。细纲确认后，**逐项检查并读取以下 15 项上下文**，每项必须实际读取并输出状态（✅已加载 / ⚠️缺失 / 🚫阻断）：
...
2. **准备层**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   ...
   - 2.5 **质量约束注入**（⚠️ 不可跳过）：写作前将以下规则加载到上下文，作为本次写作的硬约束红线。**写的时候就避开这些问题，而不是写完再改**：
      - (a) **禁用词清单**：读取 `_shared/scripts/banned-words.js` 中的 Level1（31词：不禁、竟然、居然、仿佛、宛如、恰似、犹如、嘴角勾起、嘴角上扬、眼中闪过、深吸一口气、缓缓开口、淡淡说道...）和 Level2（18词）。写作时**直接绕开这些词**，不要写完再扫描替换
      - (b) **AI腔句式禁令**：读取 `references/anti-ai-writing.md` §三中的禁止句式。写作时**直接用动作/细节替代**，不要写完再改：
        - "他感到一股X涌上心头" → 用动作/细节代替
        - "她的X宛如Y" → 删掉比喻或换新奇比喻
        - "这不仅X，更Y" → 只说一个
        - "这一刻，X" → 直接写X
        ...
        - "不是A，而是B" → 直接写B
        - 禁止排比：连续3句以上相同结构
      - (c) **段落密度规则**：一段不超过4行，一段只承载一个动作/信息变化，单句超过45字拆短，段落超过60字按句号/动作转折拆开
      - (d) **对话自然度**：加入不完整句、打断、废话，口语化，不同角色说话方式要区分。对话标签（"他说""她道"）不超过40%
      - (e) **心理描写限制**：每次不超过2句，优先用动作替代（能动手别动心）
...
> ⚠️ 去AI味不是写完再改，而是写作时就必须遵循的规则。完整指南见 `references/anti-ai-writing.md`。
