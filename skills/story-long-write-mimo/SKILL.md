...
```bash
# 首次设置
node _shared/scripts/goal.js <项目目录> --target "写到第{N}章" --min-words 3000
...
2. **读取上下文**（按需加载，缺失则跳过。可选快捷路径：如需使用子智能体批量加载上下文，通过 MiMo Code 的 actor 工具 spawn（prompt: "项目目录：{dir}\n查询类型：context_load\n查询参数：准备写第 {N} 章") 一次获取上下文）：
   ...
   - (15) `追踪/cross-chapter-fingerprint.md`（自动生成）— 前 5 章的 n-gram 指纹摘要，用于跨章重复预检
...
**AI标点清理**（写完每章后必做）：
- 运行 `node scripts/normalize-punctuation.js` 自动清理AI特殊标点
- 检查并替换智能引号（" " ' '）为直引号
