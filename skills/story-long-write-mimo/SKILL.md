...
1. **检查细纲 + 读取上下文**（⚠️ 不可跳过，不可偷懒，不可省略任何一项）：读取 `大纲/细纲_第{N}章.md`。如果不存在，**必须先补建细纲再写正文**，不允许跳过细纲直接写作。补建时参考卷纲中本章对应的事件规划和上下文。细纲确认后，**逐项检查并读取以下 28 项上下文**，每项必须实际读取并输出状态（✅已加载 / ⚠️缺失 / 🚫阻断）：

   | 编号 | 文件 | 说明 | 缺失严重度 |
   |------|------|------|-----------|
   | (1) | `正文/第{N-1}章_*.md` | 上一章正文 | 🚫 BLOCK（首章除外） |
   | (2) | `大纲/细纲_第{N}章.md` | 本章细纲（含钩子设计） | 🚫 BLOCK |
   | (3) | `追踪/伏笔.md` | 待回收伏笔 | ⚠️ WARN |
   | (4) | `设定/角色/{相关角色}.md` | 本章涉及角色 | ⚠️ WARN |
   | (5) | 对标书路径下 `拆文报告.md` | 对标参考 | ⚠️ WARN |
   | (6) | `对标/{对标书名}/原文/第{N}章_*.md` | 同位置章节参考 | ℹ️ 可选 |
   | (7) | `参考资料/{topic}.md` | 历史研究资料 | ℹ️ 可选 |
   | (8) | `追踪/角色状态.md` | 角色当前状态快照（含穿衣、物品、身体） | ⚠️ WARN |
   | (9) | `追踪/物品.md` | 关键物品当前位置、状态 | ⚠️ WARN |
   | (10) | `追踪/环境.md` | 当前季节、天气、场景位置 | ⚠️ WARN |
   ...
   | (27) | `对标/{主对标书}/文风.md` | 对标书文风（few-shot模仿） | ⚠️ WARN |
   | (28) | `拆文库/{书名}/拆文报告.md` | 可借鉴套路、写法技巧 | ⚠️ WARN |

   **对标文件加载规则**（第27-28项）：
   - 从 `设定/题材定位.md` 读取「主对标书」字段确定对标书名
   - 优先加载 `对标/{主对标书}/文风.md`（项目内引用视图），不存在则回退 `拆文库/{主对标书}/文风.md`
   - 优先加载 `对标/{主对标书}/拆文报告.md`，不存在则回退 `拆文库/{主对标书}/拆文报告.md`
   - 如果既无 `对标/` 目录也无 `拆文库/` 目录，或「主对标书」字段为空，这两项标记为跳过，不阻塞写作
...
2. **准备层**（下面的 3 步是核心方法在单章写作中的落地：筛选状态 → 召回模块 → 确认意图）：
   - 2.1 **状态筛选**：从 `追踪/角色状态.md` 中筛选本章涉及角色的当前状态，从 `追踪/伏笔.md` 中筛选本章需要回收/推进的伏笔。输出最简记忆包（参考 state-tracking.md）。如果角色状态文件不存在，从角色设定和前文推断
   - 2.2 **模块召回与文风召回**：
...
10. **更新追踪**：写完后即时更新以下追踪文件（参考 `references/consistency-tracking.md`）：
    - `追踪/伏笔.md`：新增/回收伏笔，更新紧迫度
    - `追踪/时间线.md`：记录事件时序，更新季节/时间
    - `追踪/角色状态.md`：更新角色身份、穿衣、身体状态、物品位置、关系变化，追加变更记录
    - **角色同步检查**：更新角色状态后，运行 `node skills/_shared/scripts/character-sync.js <项目目录> --json` 验证设定与追踪一致。如发现「missing_tracking」或「missing_design」问题，当场修复
...
11. **中途快照 + 质量门禁**（长篇写作安全网）：每连续写完 3 章，在继续前执行以下操作（在 Phase 5 之后）：
    ...
    - (c) **自动运行质量门禁**：`node skills/_shared/scripts/quality-gate.js <章节文件> --json`（需从全局skill目录执行）
