...
**① 语言规整流畅问题**
- 句子不要太平稳工整，每段至少有一句口语化表达
- 避免连续3句以上相同结构（长短句交替打散节奏）
- 适度加入停顿、语气词（嗯/啊/那个/行吧/算了）
- 不同角色说话习惯要区分（口癖、节奏、用词偏好）

**② 逻辑平稳无个性问题**
- 不要用"一方面……另一方面……"这种八股结构
- 角色观点要强烈、有偏向，不能永远中立
- 情绪要有起伏，不能一直平稳叙述
- 每章至少有一个"角色做出非常规选择"的时刻

**③ 举例泛化问题**
- 所有举例必须有具体细节（时间、地点、物件、人物表情）
- 禁止"比如一个人……""例如某天……"等模糊开头
- 场景描写加入五感细节（气味、温度、触感、声音）
- 角色互动加入小动作（抠指甲、抖腿、弹灰、转杯子）

**④ 立场模糊问题**
- 禁止"总体而言、在一定程度上、需结合实际情况"等模糊兜底
- 角色做出的判断必须有明确立场，不能模棱两可
- 冲突场景中，双方观点要有不可调和的矛盾点

**⑤ 高频表达重复问题**
- 每千字"总之/因此/然而/不过"不超过2次
- 禁止连续段落以相同连接词开头
- 同一章内同一表达不要重复使用3次以上

**⑥ 事实精准问题**
- 小说中的设定信息（日期、数字、地名）必须前后一致
- 如引用现实信息（法规、科学概念），必须查证后再写
- 避免模糊表述"据说……""传说中……"作为设定替代

**⑦ 结构模板化问题**
- 段落长短要参差，不要每段都是4-5行
- 避免"第一…第二…第三…"的模板式分段
- 章节之间过渡要有变化，不能每章结尾都用悬念钩子同一种方式
- 写完检查：如果一段可以整段删掉而不影响理解 → 说明是废话，删
