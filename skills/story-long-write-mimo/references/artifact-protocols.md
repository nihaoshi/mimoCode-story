...
此文件由 `story-long-analyze-mimo` 拆解管道输出（快速预览报告或完整拆文报告）。write skill 的职责是**读取**，不创建。
...
---
> 来源：{手动输入 / story-long-analyze-mimo 导入}
> 原始字数：{约N字}
```
...
**状态含义**：`未埋` = 已规划但尚未埋设；`已埋` = 已埋设且仍开放，是长篇正常状态；`已回收` = 已关闭；`已过期` = 已错过预计回收窗口，需要 `/story-review-mimo` 或显式修复。SessionStart hook 不应仅因 `未埋`、`已埋` 或 `已回收` 报警。日更流程只处理本轮新增/推进/回收的伏笔，全量伏笔审计必须由 `/story-review-mimo` 或用户明确要求触发。
