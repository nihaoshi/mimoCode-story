...
version: 1.0.0
description: |
  章节快速书写。融合 long-write 的按需加载+质量约束前置 和 chapter-write 的15步子agent隔离架构。
...
1. **子 agent 隔离执行**：15步全部子 agent 隔离，上下文完全隔离
2. **按需加载**：从细纲提取本章涉及的角色/场景/伏笔，只加载对应文件（非全量读取50+设定）
3. **最简记忆包**：从角色状态中筛选本章涉及角色，输出一句话状态
4. **质量约束前置**：写作前注入8项硬约束，写时就避开，不是写完再改
5. **综合检测**：字数、禁用词、一致性、设定校验合并为一次检测（合并 long-write 的 T-GATE + T-CONSIST）
6. **综合修复**：一个修复 agent 处理所有问题
...
│    ├── T-CHAP-{N}-05: 分析细纲+状态筛选（输出最简记忆包） [子 agent 隔离·general]
│    └── [条件] T-CHAP-{N}-07: 创建新设定文件 [子 agent 隔离·general]
│
├─── Phase 2: 写作阶段（3步）
│    ├── T-CHAP-{N}-08: 读取上下文（分层+模块召回+文风召回） [子 agent 隔离·general]
│    ├── T-CHAP-{N}-09: 生成约束参数+质量约束注入（8项硬约束） [子 agent 隔离·general]
│    └── T-CHAP-{N}-10: 正文写作（三维度织入+镜头断段+密度重排） [子 agent 隔离·general]
│
├─── Phase 3: 检测阶段（1步）
│    └── T-CHAP-{N}-11: 综合质量检测（8项） [子 agent 隔离·general]
│
├─── Phase 4: 修复阶段（2步）
...
#### Step 08: 读取上下文（融合 long-write 的文风召回+模块召回）
...
#### Step 11: 综合质量检测（合并 long-write 的 T-GATE + T-CONSIST）
...
- **防偷懒**：必须从细纲实际解析；设定文件只列出细纲涉及的，非全部50+个
...
### Step 11: 综合质量检测（合并 T-GATE + T-CONSIST）
- **Agent**: 子 agent 隔离（general）
- **检测项**：8项（合并 long-write 的 T-GATE 4项 + T-CONSIST 7项）
- **设定校验**：只读细纲涉及的设定文件
- **跨卷一致性**：只读细纲涉及的跨卷文件
...
| 维度 | chapter-write | long-write | 快速书写 |
|------|--------------|-----------|---------|
| 任务树规模 | 15步 | 62条 | 15步 |
| 上下文读取 | 全量读取50+设定 | 22个子任务全量扫描 | **按需筛选，只读细纲涉及** |
| 最简记忆包 | ❌ | ✅ | ✅ |
| 文风召回 | ❌ | ✅ | ✅ |
| 质量约束注入 | ❌ | ✅ | ✅ |
| 写作技法 | ❌ | ✅ | ✅ |
| 质量检测 | 8项综合 | T-GATE + T-CONSIST 双循环 | **8项综合（合并双循环）** |
| 跨卷追踪 | 条件加载 | 完整支持 | 条件加载 |
| 适用场景 | 细粒度控制 | 大型项目，千万字级 | **日常续写，高效单章** |
